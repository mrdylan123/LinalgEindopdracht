...
Shape::Shape(std::vector<Vector> vectors, std::vector<std::pair<Vector*, Vector*>> edges, Vector position) : vectors_{ std::move(vectors) }, edges_{ std::move(edges) }, position_{ std::move(position) }
{
}

Shape::~Shape() = default;

void Shape::render(SDL_Renderer& renderer, Camera& camera)
{
	for(auto& vector: vectors_)
	{
		vector.setX(vector.x() + 0.01);
	}

	for (const std::pair<Vector*, Vector*>& edge : edges_)
	{
		Vector v1 = camera.getWindowLocation(*edge.first);
		Vector v2 = camera.getWindowLocation(*edge.second);

		SDL_RenderDrawLine(&renderer, v1.pixelX(), v1.pixelY(), v2.pixelX(), v2.pixelY());
	}
}

void Shape::scale(int factorX, int factorY, int factorZ)
{
...
void Shape::translate(int xDirection, int yDirection, int zDirection)
{
...
void Shape::scaleFromPoint(int factorX, int factorY, int factorZ, Vector& point)
{
...
void Shape::rotateFromPoint(int degrees, Vector& point)
{
	translate(-point.x(), -point.y(), -point.z());
	rotate(degrees);
	translate(point.x(), point.y(), point.z());
...
void Shape::rotateX(float degrees)
{
...
void Shape::rotateY(float degrees)
{
	Matrix rotationMatrix = Matrix{ 4, 4 };
...
void Shape::rotateZ(float degrees)
{
	Matrix rotationMatrix = Matrix{ 4, 4 };
...
void Shape::rotateAroundAxis(int degrees, Vector& axis)
{
...
void Shape::rotateAroundRandomAxis(int degrees, Vector startPoint, Vector endPoint)
{
