...
Matrix Camera::lookatMatrix()
{
    ...
    eye_.subtract(lookAt_->position());
...
void Camera::moveX(float amount)
{
    ...
    vectors_[0].setX(position_.x());
}

void Camera::moveY(float amount)
{
    position_.setY(position_.y() + amount);
    vectors_[0].setY(position_.y());
}

void Camera::moveZ(float amount)
{
    position_.setZ(position_.z() + amount);
    vectors_[0].setZ(position_.z());
}

void Camera::zoom(float amount)
{
    Vector zoomVector{ lookAt_->position() };
    zoomVector.subtract(position_);

    zoomVector.normalize();
    zoomVector.multiply(amount);

    position_.add(zoomVector);
    vectors_[0].add(zoomVector);
}
