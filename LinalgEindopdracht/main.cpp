...
int main()
{
	...
	if (SDL_CreateWindowAndRenderer(600, 600, 0, &window, &renderer) == 0) {
		...
		while (SDL_PollEvent(&event)) {
			...
			case SDLK_UP: graph.camera().moveZ(-0.1); break;
			case SDLK_DOWN: graph.camera().moveZ(0.1); break;
			case SDLK_LEFT: graph.camera().moveX(-0.1); break;
			case SDLK_RIGHT: graph.camera().moveX(0.1); break;
			case SDLK_KP_PLUS: graph.camera().zoom(0.1); break;
			case SDLK_KP_MINUS: graph.camera().zoom(-0.1); break;
			case SDLK_PAGEUP: graph.camera().moveY(0.1); break;
			case SDLK_PAGEDOWN: graph.camera().moveY(-0.1); break;
			}
		    }
		}
	    }
